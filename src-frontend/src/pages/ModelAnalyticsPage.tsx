...
function MetricCard({
  ...
  return (
    <div
      className={cn(
        'border-2 p-4',
        accent ? 'border-primary bg-primary/5' : 'border-border bg-surface',
      )}
    >
      ...
      <div
        className={cn(
          'mt-3 font-mono text-3xl font-black tabular-nums',
...
function HorizontalBar({
  ...
  return (
    <div className="flex items-center gap-3">
      <span
        className={cn(
          'w-28 shrink-0 truncate text-right text-[11px]',
          mono ? 'font-mono text-text-muted' : 'text-text-secondary',
        )}
      >
        {label}
      </span>
...
export function ModelAnalyticsPage() {
  ...
  const assistantRate =
    analytics && analytics.message_count > 0
      ? (analytics.assistant_message_count / analytics.message_count) * 100
      : 0
  const attachmentPerMessage =
    analytics && analytics.message_count > 0
    ...
    <div className="flex h-full">
      ...
      <aside className="w-72 shrink-0 border-r-2 border-border bg-surface-dim">
        <div className="border-b-2 border-border p-4">
          ...
          <div className="mt-4">
            <h1 className="font-mono text-xs font-black uppercase tracking-[0.18em] text-text">
              Analytics
            </h1>
              ...
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="shrink-0 border border-border bg-surface-dim px-4 py-3 text-right">
                  ...
                  <div className="mt-0.5 text-sm font-semibold text-text">
                    {formatDate(selectedModel.added_at)}
                  </div>
                  {selectedModel.last_used && (
                    <>
                      ...
                      <div className="mt-0.5 text-sm font-semibold text-text">
                        {formatDate(selectedModel.last_used)}
                      </div>
                    </>
                  )}
                </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">
                ...
                <MetricCard
                  ...
                  label="Avg Response"
                  value={
                    analytics?.avg_generation_time_ms != null
                ...
                <MetricCard
                  ...
                  value={
                    analytics?.tokens_per_second != null
                      ? `${analytics.tokens_per_second.toFixed(1)}/s`
                      : '—'
                  }
                  loading={analyticsLoading}
                />
              ...
              <div className="border-2 border-border bg-surface p-5">
                ...
                <div className="space-y-4 mb-5">
                  ...
                  <div className="space-y-3">
                    <RatioBar
                      label="Assistant message rate"
                      value={`${assistantRate.toFixed(1)}%`}
                      percent={assistantRate}
                    />
                    <RatioBar
                ...
                <div className="space-y-5">
                  <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">
                    {[
                      ...
                      { label: 'Parameters', val: inspectionData.inspection.model_params ?? '—' },
                      {
                        label: 'Context',
                        val:
                          inspectionData.inspection.context_length != null
                            ? `${inspectionData.inspection.context_length.toLocaleString()} tok`
                            : '—',
                      },
                  ...
                  <div className="border border-border bg-surface-dim px-4 py-3">
                    ...
                    <div className="mt-2 break-all font-mono text-xs text-text-secondary">
                      {inspectionData.inspection.command}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="grid gap-1.5 md:grid-cols-2 xl:grid-cols-3">
                      {inspectionData.inspection.metadata.slice(0, 18).map((entry) => (
                        <div
                          key={entry.key}
                          className="border border-border bg-surface-dim px-3 py-2"
                        >
                          <div className="font-mono text-[9px] uppercase tracking-widest text-text-muted">
                            {entry.key}
                          </div>
                          <div className="mt-0.5 break-words text-xs font-medium text-text">
                            {entry.value}
                          </div>
                        </div>
                      ))}
            ...
            <div className="border-2 border-border bg-surface">
              ...
              {recentConvs.length > 0 ? (
                <div>
                  <div className="grid grid-cols-[1fr_80px_80px_80px] border-b border-border bg-surface-dim px-5 py-2">
                    {['Title', 'Responses', 'Tokens', 'Updated'].map((h) => (
                      <div
                        key={h}
                        className="font-mono text-[9px] uppercase tracking-widest text-text-muted"
                      >
                        {h}
                      </div>
                    ))}
                  ...
                  {recentConvs.map((conv) => (
                    ...
                    >
                      <div className="min-w-0 truncate text-sm font-medium text-text">
                        {conv.title}
                      </div>
                ...
                <p className="px-5 py-8 text-center text-sm text-text-muted">
                  No tracked conversations for this model yet.
                </p>
              )}
            </div>
          </div>
